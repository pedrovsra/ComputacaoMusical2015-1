 ...
 * McGill Univarsity
 */
package AudioLibs.Classifier.Fast.featureExtraction;
...
import AudioLibs.Classifier.Fast.GeneralTools.Statistics;
...
import AudioLibs.Classifier.Fast.featureExtraction.Aggregators.AggregatorContainer;
...
public class FeatureProcessor {
     ...
     *
     * @param window_size The size of the windows that the audio recordings are
     * to be broken into.
     * @param window_overlap The fraction of overlap between adjacent windows.
     * Must be between 0.0 and less than 1.0, with a value of 0.0 meaning no
     * overlap.
     * @param sampling_rate The sampling rate that all recordings are to be
     * converted to before feature extraction
     * @param normalise Whether or not to normalise recordings before feature
     * extraction.
     * @param all_feature_extractors All features that can be extracted.
     * @param features_to_save_among_all Which features are to be saved. Entries
     * correspond to the all_feature_extractors parameter.
     * @param save_features_for_each_window Whether or not to save features
     * individually for each window.
     * @param save_overall_recording_features Whether or not to save the average
     * and standard deviation of each feature accross all windows.
    ...
    /* PUBLIC METHODS ********************************************************* */
    /**
     * Extract the features from the provided audio. Splitting windows as part
     * of the process.
     *
     * @param samples The audio samples to extract features from.
     */
     ...
     * @param window_start_indices The indices of samples that correspond to
     * where each window should start.
     * @return The extracted feature values for this recording. The first indice
     * identifies the window, the second identifies the feature and the third
     * identifies the feature value. The third dimension will be null if the
     * given feature could not be extracted for the given window.
     ...
     * each such feature.
     *
     * @param window_feature_values The extracted window feature values for this
     * recording. The first indice identifies the window, the second identifies
     * the feature and the third identifies the feature value. The third
     * dimension will be null if the given feature could not be extracted for
     * the given window.
     * @return The extracted overall average and standard deviations of the
     * window feature values that were passed to this method. The first indice
     * identifies the feature and the second iddentifies the feature value. The
     * order of the features correspond to the FeatureDefinitions that the
     * overall_feature_definitions parameter is filled with.
     */
        ...
        for (int feat = 0; feat < feature_extractors.length; feat++) {
            if (window_feature_values[window_feature_values.length - 1][feat] != null
                ...
                for (int val = 0; val < window_feature_values[window_feature_values.length - 1][feat].length; val++) {
                    ...
                    if (mean) {
                        averages[val] = Statistics
                                .getAverage(values_to_process);
                    }
                    if (stdev) {
                        stdvs[val] = Statistics
     ...
     * features to disk.
     *
     * @param feature_values The extracted feature values for this recording.
     * The first indice identifies the window, the second identifies the feature
     * and the third identifies the feature value. The third dimension will be
     * null if the given feature could not be extracted for the given window.
     * @param window_start_indices The indices of samples that correspond to
     * where each window should start.
     * @param identifier A string to use for identifying this recording. Often a
     * file path.
     * @param overall_feature_values The extracted overall average and standard
     * deviations of the window feature values. The first indice identifies the
     * feature and the second identifies the feature value. The order of the
     * features correspond to the overall_feature_definitions parameter. This
     * value is null if overall feature values were not extracted.
     * @param overall_feature_definitions The feature definitions of the
     * features that are in the overall_feature_values parameter. Will be null
     * if no overall features were extracted.
     ...
     * definitions.
     *
     * @param feature_values The extracted feature values for a recording. The
     * first indice identifies the window, the second identifies the feature and
     * the third identifies the feature value.
     * @param overall_feature_definitions The feature definitions of the
     * features that are in the features for the recording. Will be null if no
     * overallfeatures were extracted.
     * @throws Exception Throws an exception if cannot write.
     */
    /*
